...
class MessageDataVisitor implements IVisitor {
	...
	protected MessageBodyStructure createMessageBodyStructure(LinkedList<ConCell> bsList, int level, String parentPartSpec, int partSpec) {
		...
		if(bsList != null) {
			...
			BodyStructure subBs = createBodyStructure(cell,level+1,parentPartSpec,partSpec); //<<<
	...
	public BodyStructure createBodyStructure(ConCell bs, int level, String parentPartSpec, int partSpec) {
		if (bs != null) {
			...
			if(cellList.getFirst().getList() == null) {
				...
				if (mediaType.equals("TEXT")) {
					...
					_bs = createMessageBodyStructure(cellList, level, parentPartSpec, partSpec);
				} else {
				...
				_bs.setFields(flds);
				_bs.setLevel(level);
				_bs.setPartSpec(parentPartSpec);
				_bs.addPartSpec(partSpec);
				return _bs;
			} else {
				BodyStructureMPart muiltBs = new BodyStructureMPart();
				muiltBs.setLevel(level);
				muiltBs.setPartSpec(parentPartSpec);
				muiltBs.addPartSpec(partSpec);
				for (int i=0; i<cellList.size(); ++i) {
					ConCell c = cellList.get(i);
					if (c.getList() != null) {
						if (c.getList().getFirst().getList() != null) {
							// has sub part
							BodyStructure sub = createBodyStructure(c, level+1, muiltBs.getPartSpec(), i+1);
							muiltBs.addBodyStructure(sub);					
							continue;
						} else {	
							BodyStructure _bs = createBodyStructure(c, level+1, muiltBs.getPartSpec(), i+1);
	...
	public BodyStructure createBs() {
		...
		if(cell != null) {
			return createBodyStructure(cell, 0, null, 0);
		}
		return null;
	}
}