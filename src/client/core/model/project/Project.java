...
public abstract class Project extends Task implements EventListener, ISubscribable {
	...
	public class StartEvent extends Event {
		public StartEvent() {
			setFrom(getSelfListenerGroup());
		}
	}
	
	/**
	 * Finish a running project, if the project not started or had finished , nothing would be happened.
	 */
	...
	public Project(String id) {
		setId(id);
		// the event to
		setEventTo(new Notifiers(getSelfListenerGroup()));
	...
	final protected Event process() {
		if(mTodoList.isEmpty()) {
			// no sub task, return
			Core.I().removeListenerGroup(getSelfListenerGroup());
			return null;
		}
		onStart();		
	...
	public void onEvent(Event event) {
		if(event.isFrom(getSelfListenerGroup())) {
		} 
		onLaunchTask(event);
	}
	...
	public void onLaunchTask(Event triggerEvent) {
		if(mTodoList.isEmpty()) {
			...
			if(todo != null) {
				...
				if(task != null) {
					task.addEventTo(getSelfListenerGroup());
					Core.I().exec(task);
					mTodoList.remove(0);
